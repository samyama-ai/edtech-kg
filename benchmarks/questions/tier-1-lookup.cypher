...
//   — the schema declares keys and these are not among them, so this
//     parses and reaches for something no loader writes. See #123.
MATCH (cm:Completion)-[:AT]->(i:Institution)
...
MATCH (i:Institution)-[:LOCATED_IN]->(pl:Place)
WHERE i.unitid = "100654"
...
MATCH (pw:Pathway)-[:INCLUDES]->(c:Course)
WHERE pw.kind = "career pathway"
...
//   — the schema declares keys and these are not among them, so this
//     parses and reaches for something no loader writes. See #123.
MATCH (er:EarningsRecord)-[:FOR_OCCUPATION]->(o:Occupation)
WHERE o.soc_code = "15-1252"
