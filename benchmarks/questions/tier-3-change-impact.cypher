...
MATCH (p:Programme)-[:PREPARES_FOR]->(:Occupation {soc_code: "29-1141"})
RETURN p.cip_code, p.name;
...
MATCH (dependant:Course)-[:REQUIRES]->(gone:Course {url: "https://catalog.pwcs.edu/agriculture/landscaping-1"})
...
MATCH (downstream:Course)-[:REQUIRES*]->(:Course {url: "https://catalog.pwcs.edu/agriculture/landscaping-1"})
...
MATCH (p:Programme {cip_code: "11.0101"})
...
//   really testing for.
MATCH (closing:Institution {unitid: "100654"})-[:LOCATED_IN]->(pl:Place)
...
MATCH (:School {ncessch: "510126000341"})-[:TEACHES]->(c:Course {url: "https://catalog.pwcs.edu/agriculture/landscaping-1"})
...
MATCH (unlocked:Course)-[:REQUIRES*]->(:Course {url: "https://catalog.pwcs.edu/agriculture/landscaping-1"})
...
MATCH (dependant:Course)-[:REQUIRES]->(renamed:Course {url: "https://catalog.pwcs.edu/agriculture/landscaping-1"})
