...
#: is enough for the 958-page sweep to survive one, and short enough that a
#: service genuinely down is not hammered. A status code is never retried: that
#: is the service answering, and asking again gets the same answer.
ATTEMPTS = 3
BACKOFF = 2.0
...
def _request(url: str, method: str, extract):
    ...
    # service answering, and asking again gets the same answer — but a
    # connection dropped mid-body is transport, and the 958-page sweep hit one
    # at page 75 after twenty minutes of walking.
    for attempt in range(ATTEMPTS):
        try:
            ...
            # bare exception while every other failure on this path arrived as
