...
def test_deleted_and_provisional_are_measured_not_assumed(monkeypatch):
    ...
    headers_stub(monkeypatch, {
        f"{read.REGISTRY}/ce-registry/search?per_page=1&include_deleted=only": 7,
        f"{read.REGISTRY}/ce-registry/search?per_page=1&provisional=only": 3,
        "/search": 100})
    r = probe.registry_totals()
...
def test_an_api_root_that_is_not_an_object_is_malformed(monkeypatch):
    ...
    monkeypatch.setattr(read.urllib.request, "urlopen", lambda *a, **k: R())
    with pytest.raises(read.MalformedSource, match="not an object"):
