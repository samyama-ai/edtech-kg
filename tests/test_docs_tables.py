...
def test_no_document_uses_a_table_style_this_check_cannot_read(document):
    ...
    """
    unreadable = _unreadable_tables(document.read_text(encoding="utf-8"))
    assert not unreadable, (
        f"{document.name}: table(s) at line(s) {[n for n, _ in unreadable]} "
        f"are written without outer pipes, which this check cannot read. Add "
        f"the outer pipes so stranded rows in them would be caught — "
        f"{unreadable[0][1][:70]}")


def test_a_table_without_outer_pipes_is_refused(tmp_path):
    """The check above is vacuous against this corpus — no document uses the
    form, so its assert body is never reached and neutering its logic leaves
    the suite green. The rule is pinned here on a crafted document instead,
    which is the shape the orphan check already uses."""
    assert _unreadable_tables(
        "Heading\n\n"
        "Question | Answer\n"
        "---|---\n"
        "How many | 791\n") == [(3, "Question | Answer")]
...
def test_an_ordinary_table_is_not_refused(tmp_path):
    """The other direction. A checker that rejects everything is not a
    checker, and this one runs over every document in the repo.

    Note what this does NOT reach: a fully piped separator is itself a row,
    so the loop skips it before the header is ever examined. Written as the
    only false-positive test it passes while the branch below it rejects
    everything — which it did. The test under this one is the one with teeth.
    """
    assert _unreadable_tables(
        "Heading\n\n"
        "| Question | Answer |\n"
...
def test_a_separator_missing_only_its_own_pipes_is_not_refused(tmp_path):
    """The header carries outer pipes and the separator does not.

    This is the case that actually reaches the header test, so it is what
    stops the rule widening to "any line above a bare separator". Stranded
    rows in a table this shape ARE caught by the orphan check, because its
    rows are readable — only the separator is written short.
    """
    assert _unreadable_tables(
        "Heading\n\n"
        "| Question | Answer |\n"
...
def test_a_separator_inside_a_code_block_is_not_a_table(tmp_path):
    """`---|---` inside a fence is sample output, not markup this check owns."""
    assert _unreadable_tables(
        "```\n"
        "Question | Answer\n"
        "---|---\n"
...
def _unreadable_tables(text: str) -> list[tuple[int, str]]:
    """Line number and header of every table written without outer pipes.

    Detected by its separator: a `|---|` line whose row above has no outer
    pipes is a table in the form `_is_row` cannot recognise without also
    matching prose.
    """
    lines = text.splitlines()
    found = []
    for index, line in _code_free_lines(text):
        if not SEPARATOR.match(line) or _is_row(line):
            continue
        above = lines[index - 1].strip() if index else ""
        if above and not _is_row(above):
            found.append((index, above))
    return found
